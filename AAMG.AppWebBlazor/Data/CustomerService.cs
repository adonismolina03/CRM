...
namespace AAMG.AppWebBlazor.Data
{
    public class CustomerService
    {
        ...
        {
            var response = await _httpClientAAMGAPI.PostAsJsonAsync("/customer/search", queryCustomerDTO);
        ...
        {
            var response = await _httpClientAAMGAPI.GetAsync("/customer/" + id);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            int result = 0;
            var response = await _httpClientAAMGAPI.PostAsJsonAsync("/customer", createDTO);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            int result = 0;
            var response = await _httpClientAAMGAPI.PutAsJsonAsync("/customer", editDTO);
            if (response.IsSuccessStatusCode)
            {
                var responseBody = await response.Content.ReadAsStringAsync();
                if (int.TryParse(responseBody, out result) == false)
        ...
        {
            int result = 0;
            var response = await _httpClientAAMGAPI.DeleteAsync("/customer/" + id);
            if (response.IsSuccessStatusCode)
            {
                var responseBody = await response.Content.ReadAsStringAsync();
                if (int.TryParse(responseBody, out result) == false)
